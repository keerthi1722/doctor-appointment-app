...
const Notification = () => {
   ...
   const handleAllMarkRead = async () => {
      try {
         const res = await axios.post('http://localhost:5002/api/user/getallnotification', { userId: user._id }, {
            headers: {
   ...
   const handledeleteAllMark = async () => {
      try {
         const res = await axios.post('/api/user/deleteallnotification', { userId: user._id }, {
            headers: {
               Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
         })
